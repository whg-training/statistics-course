...
This means we can model the rows as binomially distributed.  Let <img src="https://render.githubusercontent.com/render/math?math=\theta_1"></img> and <img src="https://render.githubusercontent.com/render/math?math=\theta_2"></img> be the true allele frequencies of the 'C' allele in nonexposed and exposed populations.  Then we could think of the table as being generated by two binomial distributions:
```
...
One way to think of this is that the likelihood is very certain that ϑ₁ is not zero (as it must be since there's a count of at least 1) but not so certain about just how big it is.  In any case - we should be cautious about the value of the standard error or of P-values based on asymptotic approximations here.

For example - let's look directly at a binomial test of the top row for the null model where 
```R
...
Here, the asymptotic confidence intervals extend below zero (impossible!) and only go up to 0.015 - reflecting how the normal approximation compares to the true loglikelihood in the plot.
